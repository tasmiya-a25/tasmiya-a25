...
# Info items (Two Column Layout for wide terminal)
INFO_COL1 = [
    ("User",     "Tasmiya A"),
...
INFO_COL2 = [
    ...
    ("Infra",    "Docker, PostgreSQL, K8s"),
]

specs_str = ""
y_spec_1 = 110
for i, (key, value) in enumerate(INFO_COL1):
    delay = 1.0 + i * 0.15
    specs_str += f"""
  <g opacity="0">
    ...
    <text x="40" y="{y_spec_1}" font-family="Consolas, monospace" font-size="14" font-weight="bold" fill="#ff9ecd">{key}:</text>
    <text x="130" y="{y_spec_1}" font-family="Consolas, monospace" font-size="14" fill="#f0d6ff">{value}</text>
  </g>"""
    y_spec_1 += 40

y_spec_2 = 110
for i, (key, value) in enumerate(INFO_COL2):
    delay = 1.0 + (len(INFO_COL1) + i) * 0.15
    specs_str += f"""
  <g opacity="0">
    <animate attributeName="opacity" from="0" to="1" begin="{delay}s" dur="0.4s" fill="freeze"/>
    <animateTransform attributeName="transform" type="translate" from="0 8" to="0 0" begin="{delay}s" dur="0.4s" fill="freeze"/>
    <text x="330" y="{y_spec_2}" font-family="Consolas, monospace" font-size="14" font-weight="bold" fill="#ff9ecd">{key}:</text>
    <text x="400" y="{y_spec_2}" font-family="Consolas, monospace" font-size="14" fill="#f0d6ff">{value}</text>
  </g>"""
    y_spec_2 += 40

# Integrated inline badges under specs (centered)
badges_str = f"""
  <g opacity="0" transform="translate(172, 230)">
...
svg_content = f"""<svg xmlns="http://www.w3.org/2000/svg"
  ...
  <g class="glow-card" transform="translate(310, 15)">
    <rect width="625" height="320" rx="12" fill="#10051a" fill-opacity="0.85" stroke="#4a1a36" stroke-width="1.5"/>
    <g>
      <circle cx="18" cy="18" r="5" fill="#ff5f56" />
      <circle cx="33" cy="18" r="5" fill="#ffbd2e" />
      <circle cx="48" cy="18" r="5" fill="#27c93f" />
      <text x="312.5" y="18" font-family="Consolas, monospace" font-size="11" fill="#8b949e" text-anchor="middle" dominant-baseline="middle">tasmiya@core:~</text>
      <line x1="0" y1="36" x2="625" y2="36" stroke="#4a1a36" stroke-width="1"/>
    </g>
    <g transform="translate(0, 0)">
      <text x="40" y="65" font-family="Consolas, monospace" font-size="16" font-weight="bold" fill="#ff9ecd">sys_info()</text>
    </g>
    {specs_str}
    {badges_str}
